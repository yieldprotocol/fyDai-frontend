...
function PaybackAction({ close }:RepayActionProps) {
  ...
  const [buyType, setBuyType] = React.useState<string>('Dai');
      ...
      <Box justify='center'>
        <Box round background='border' pad={{ horizontal:'small' }}>
          <Select
            ...
            icon={false}
            onChange={(e:any) => {e.stopPropagation(); setBuyType(e.option);}}
          />
        </Box>
      </Box>
    );
  ...
  return (
    ...
    <Box flex='grow' justify='between'>
      <Box margin={{ top:'medium' }} gap='xsmall' align='center' fill='horizontal'>
        <Text alignSelf='start' size='xsmall'> Amount to lend</Text>
        <Box 
          round='small'
          border={{ color:'brand' }}
          direction='row'
