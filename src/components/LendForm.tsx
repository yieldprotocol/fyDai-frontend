...
function LendForm({ series, closeLayer }:LendConfirmLayerProps) {
  ...
  const { state: notiState, dispatch: notiDispatch } = React.useContext(NotifyContext);
  // const { state: posState, dispatch: posDispatch } = React.useContext(PositionsContext);
  ...
  const success = () => {
    notiDispatch({ type: 'notify', payload:{ message:'Transaction processed', type:'success' } } );
    // posDispatch({ type:'indicatorPlus' });
  };

  const handleConfirm = async () => {
    closeLayer();
    notiDispatch({ type: 'notify', payload:{ message:'Transaction pending....', type:'info', showFor:4000 } } );
    await setTimeout(() => {
      success();
    }, 3000);
  };
