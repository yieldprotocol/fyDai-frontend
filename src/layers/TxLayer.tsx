...
const TxLayer = () => {
  ...
  const {
    state: { requestedSigs, pendingTxs, txProcessActive, fallbackActive },
    dispatch,
  } = useContext(TxContext);
  const {
    state: { authorization, preferences },
    actions: userActions,
  ...
  useEffect(() => {
    if (Array.from(requestedSigs.values()).length) {
      const _allSigned = Array.from(requestedSigs.values()).every(
        (x: any) => x.signed === true
      );
      setAllSigned(_allSigned);

      const _allComplete = Array.from(requestedSigs.values()).every(
        (x: any) => x.complete === true
      );
  ...
  useEffect(() => {
    txProcessActive &&
      setProcessIsCurrentTx(txActive?.txCode === txProcessActive);
    ...
    <>
      {
        ...
        account && (
          <Box
            ...
            pad={
              mobile
                ? { horizontal: 'medium', top: 'medium', bottom: 'large' }
          ...
          >
            <Box gap='medium' direction='row-responsive' justify='between'>
              <Text size={mobile ? 'xsmall' : undefined}>
                As of 31 December 2021, this version of Yield (v1) has been
                depreciated.
              </Text>
              <RaisedButton
                background="#555555"
                label={
                  <Box
                    ...
                    align="center"
                  >
                    <Text size="small" color="#DDDDDD">
                      Check out V2
                    </Text>
                  </Box>
                }
                onClick={() => window.open('https://app.yieldprotocol.com/')}
              />
            </Box>
          ...
          >
            ...
            >
                  ...
                  <>
                    <Box gap="medium">
                      {allSigned ? (
                        <Text weight="bold">
                          {' '}
                          The required authorizations have been granted:{' '}
                        </Text>
                      ) : (
                        <Text weight="bold">
                          {' '}
                          The following{' '}
                          {requestedSigs.length === 1
                            ? 'signature is'
                            : 'signatures are'}{' '}
                          required:{' '}
                        </Text>
                      ...
                      {!processIsCurrentTx && allSigned && (
                        <Text>
                          Finally, please check your wallet or provider to
...

              {
                ...
                fallbackActive && !allComplete && requestedSigs.length > 0 && (
                  <Box gap="medium">
                    {preferences?.useTxApproval ? (
                      <>
                        <Text weight="bold">
                          {' '}
                          {!allSigned
                            ? 'Please approve the following authorization transactions:'
                            : 'The required authorizations have all been granted:'}{' '}
                        </Text>
                      </>
                    ) : (
                      <>
                        <Box gap="medium">
                          <Box>
                            <Text weight="bold">
                              It seems there was a problem signing the
                              authorization.
                            </Text>
                            {!mobile && (
                              <Text size="xsmall">
                                ( Its not your fault, some wallets don't provide
                                signing functionality just yet :| )
                              </Text>
                            )}
                          </Box>
                          <Box>
                            <Text size="small">
                              You can continue by approving the set of
                              authorization transactions individually with your
                              wallet or provider:
                            </Text>
                          </Box>
                        </Box>
                        ...
                        requestedSigs.map((x: any, i: number) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <Box basis="30" alignSelf="end">
                                {pendingTxs.some(
                                  (tx: any) => tx.txCode === x.id
                                ) ? (
                                  <Box gap="small" direction="row">
                                    <Text size="xxsmall">Pending</Text>
                                    <Loading condition={true} size="xxsmall">
                                      .
                                    </Loading>
                                  </Box>
                                ) : (
                      ...
                      !preferences?.useTxApproval && (
                        <Box>
                          <CheckBox
                            ...
                            label={
                              <Text size="xsmall">
                                In future, always use individual transactions
                                for authorization
                              </Text>
                            }
                            onChange={(e: any) =>
                          ...
                          <Box margin={{ left: 'large' }}>
                            <Text size="xxsmall">
                              (You can always change back to using permit-style
                              authorization in the settings)
                            </Text>
                          </Box>
                        </Box>
                      )
...

              {
                ...
                allComplete && !processIsCurrentTx && (
                  <Box gap="medium">
                    <Text weight="bold">Confirmation required</Text>
                    <Text>
                      Please check your wallet or provider to approve the
                      transaction.
                    </Text>
                  </Box>
                )
                  ...
                  <>
                    ...
                    <Box direction="row" justify="between">
                      ...
                      <FlatButton
                        ...
                        onClick={() =>
                          dispatch({ type: 'forceClear', payload: null })
                        }
                      />
                    </Box>
                  </>
